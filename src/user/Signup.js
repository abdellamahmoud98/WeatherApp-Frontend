...
const Signup = () => {
  ...
  const onFormSubmit = async (values, actions) => {
    ...
    if (apiResponse.status === 1) {
      toast(`Congratulations on successfully signing up!!. Please check your 
        ${values.email} for your verification link`);
      actions.resetForm();
      } else {
  ...
  const SignupSchema = Yup.object().shape({
    ...
    password: Yup.string()
      ...
      .required("Password cannnot be blank"),
  });
        ...
        <div className="w-full px-6 py-4 mt-6 overflow-hidden bg-white shadow-md sm:max-w-lg sm:rounded-lg">
          ...
          <div className="my-6 space-y-2">
            <div className="flex items-center justify-center w-full p-2 space-x-4 border rounded-md focus:ring-2 focus:ring-offset-1 dark:border-gray-400 focus:ring-violet-400">
              <p>Powered by</p>
              <img className="w-20 h-10" src={BPTN} alt="BPTN Logo" />
            </div>
          </div>
        </div>
      </div>
