...
const WeatherData = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (inputValue !== "") {
    // set location
    setLocation (inputValue); setSave(true); // save the weather record to the database when user search for a city
    }
...
useEffect(() => {
  ...
  if (!data) {
    return (
      <div className="w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-coljustify-center items-center"></div>
        <div>
          < LoadingIndicator />
      </div>
    </div>
